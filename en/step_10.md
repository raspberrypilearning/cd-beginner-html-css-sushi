...
- Find the file `blank_page.html` and copy and paste all of the code from it into your new file. Since you want to copy the whole thing, you can click anywhere on the code and use the keyboard shortcut `Ctrl` (or `cmd`) and `A` to select all of it at once.
...
- When there are too many tabs to be able to see them all, you can use the **<** and **>** icons in the top left-hand corner of the tabs to scroll between them.
...
- Change the value of the `href` attribute for each link (remember, that's the text inside the quotation marks.) so that it exactly matches the name of each new HTML file that you have created. 
